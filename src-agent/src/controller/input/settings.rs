...
/// larger ones now live as individual functions below (extract-arms, file size)
/// — `handle_settings` keeps calling them in the EXACT same precedence order,
/// returning early per level exactly as before. Pure code motion, no behaviour
/// change.
...
fn handle_role_picker(s: &mut SettingsState, key: KeyEvent) {
    match key.code {
        KeyCode::Up => {
            s.mm_role_picker_up();
        }
...
fn handle_model_modal(s: &mut SettingsState, rest: &mut AppStateRest, key: KeyEvent) -> Action {
...
fn handle_provider_modal(s: &mut SettingsState, key: KeyEvent) -> Action {
    match key.code {
        KeyCode::Esc => {
...
fn handle_fs_picker(s: &mut SettingsState, key: KeyEvent) -> Action {
    match key.code {
...
/// remove (min-1 rule); Enter edits the entry (opens the picker, seeded);
/// Esc returns to field navigation.
fn handle_list_editing(s: &mut SettingsState, key: KeyEvent) -> Action {
    match key.code {
...
/// `Action::None`.
fn handle_providers_nav(s: &mut SettingsState, key: KeyEvent) -> Action {
    match key.code {
        KeyCode::Esc => {
            s.focus_sidebar();
        }
...
fn handle_models_nav(s: &mut SettingsState, rest: &mut AppStateRest, key: KeyEvent) -> Action {
