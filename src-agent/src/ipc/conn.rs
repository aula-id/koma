...
// ---------------------------------------------------------------------------
// Stdio transport — for `koma server` (remote dev over SSH/piped process).
//
// Mirrors [`spawn`] / [`read_loop`] / [`write_loop`] but operates on generic
// `AsyncRead + AsyncWrite` instead of unix-specific `IpcReadHalf` /
// `IpcWriteHalf`. The IPC protocol (length-prefixed JSON frames) is identical;
// only the transport layer differs.
// ---------------------------------------------------------------------------

/// Spawn a per-client connection task over generic async stdio streams.
///
/// Unlike [`spawn`] which takes a unix [`IpcStream`], this accepts any
    ...
    W: tokio::io::AsyncWrite + Unpin + Send + 'static,
{
    ...
    let read_hub_tx = hub_tx.clone();
    tokio::spawn(async move {
        read_loop_stdio(reader, client_id, read_hub_tx).await;
    });

    tokio::spawn(async move {
        write_loop_stdio(writer, frame_rx).await;
    });
}

/// Read framed [`ClientRequest`]s from a generic async reader and forward each as
/// a [`HubInbound::Request`]. On EOF / cap violation / decode error, signal
/// [`HubInbound::Disconnect`] exactly once and end. Mirrors [`read_loop`].
async fn read_loop_stdio<R: tokio::io::AsyncRead + Unpin>(
    mut reader: R,
    client_id: u64,
    ...
    loop {
        let step: ControlFlow<()> =
            match frame::read_frame_from(&mut reader, &mut frame_reader).await {
                Ok(bytes) => match serde_json::from_slice::<ClientRequest>(&bytes) {
                    Ok(req) => {
                        if hub_tx.send(HubInbound::Request { client_id, req }).is_ok() {
                            ControlFlow::Continue(())
...
/// Drain a client's [`DaemonFrame`] queue to a generic async writer on a short
/// interval, until the queue closes or a write fails. Mirrors [`write_loop`].
async fn write_loop_stdio<W: tokio::io::AsyncWrite + Unpin>(
    mut writer: W,
