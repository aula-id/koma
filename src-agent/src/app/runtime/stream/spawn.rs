...
pub(crate) fn build_tool_ctx(state: &AppState, sess_idx: usize) -> crate::tool::ToolCtx {
    ...
    // The media download directory for web_download files. Created on access
    // so the tool can write into it without a separate create_dir_all.
    let download_dir = session_ref
        .as_ref()
        .and_then(|s| {
            crate::model::store::session_media_dir(&s.pwd_hash).ok()
        });

    // Ensure the media dir exists so web_download can write into it.
