...
/// Pure, stateless function that returns the mode/phase-specific lifecycle
/// tool names to *append* to the advertised tool list. The base list is
/// `main_tool_names()` (+ MCP/security); this function decides which
/// INTERNAL_ONLY tools (if any) the current mode+phase wants.
///
/// # Exact matrix
///
/// | AgentMode | sdlc_phase   | Appended tools                                           |
/// |-----------|------------- |---------------------------------------------------------|
/// | Auto      | *            | `plan_enter`                                             |
/// | Normal    | *            | `plan_enter`                                             |
/// | Yolo      | *            | `plan_enter`                                             |
...
///
/// The returned names are never in `main_tool_names()` (they live in
/// `INTERNAL_ONLY`), so callers that append-then-retain on the advertise
/// list never produce duplicates.
pub(crate) fn mode_advertised_lifecycle_tools(
...
/// True when a tool name belongs to any SDLC lifecycle set.
pub(crate) fn is_sdlc_lifecycle_tool(name: &str) -> bool {
    SDLC_LIFECYCLE_TOOLS.contains(&name)
}
...
over sec_remote (stateful socket).\n",
    ...
    let sdlc_phase = state.rest.sessions[sess_idx].sdlc_phase.as_deref();
    ...
    // Mode-gated lifecycle tools. Use the pure helper so the matrix is
    // independently testable. Plan/SDLC assess fold the advertise list down
    // to the read-only / reasoning / delegation whitelist first.
    {
        let lifecycle = mode_advertised_lifecycle_tools(mode, sdlc_phase);
        // Lifecycle names are phase-scoped and must come only from the matrix,
    ...
    // Provider schema generation requires unique function names. Preserve the
...
#[cfg(test)]
#[path = "run_ext_context_tests.rs"]
mod ext_context_tests;

#[cfg(test)]
#[path = "run_skill_tail_tests.rs"]
mod skill_tail_tests;

// ── Mode-advertised lifecycle tool matrix tests ─────────────────────────────

#[cfg(test)]
#[path = "run_lifecycle_tool_matrix_tests.rs"]
mod lifecycle_tool_matrix_tests;
