...
fn parse_panel_push_ignores_extra_fields() {
    ...
    assert_eq!(parsed, Some(("p1".to_string(), serde_json::json!({ "x": 1 }))));
}
...
fn enforce_cap_holds_across_interleaved_push_cycles() {
    ...
    for i in 0..200 {
        route_ext_notify(&mut out, notify("panel.push", serde_json::json!({ "panelId": format!("p{i}"), "payload": i })));
    }
    enforce_ext_panel_cap(&mut out);
    assert_eq!(out.len(), 200, "under cap: enforce must not shed anything");
    assert_eq!(out[0].1, "p0", "no shedding yet — the oldest entry is still p0");
    ...
    assert_eq!(out[0].1, "p44", "the second enforce must shed exactly the newly-oldest entries");
    ...
    enforce_ext_panel_cap(&mut out);
    assert_eq!(out.len(), EXT_PANEL_PUSH_CAP);
    assert_eq!(out[0].1, "p54", "tick 3 must shed only the entries that aged out since tick 2");
    assert_eq!(out[EXT_PANEL_PUSH_CAP - 1].1, "p309");
    // Nothing from the first-ever shed window (p0..=p43) can have resurfaced.
    assert!(out.iter().all(|(_, id, _)| id.as_str() != "p0" && id.as_str() != "p43"));
}
