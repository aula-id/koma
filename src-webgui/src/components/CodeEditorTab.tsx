...
export default function CodeEditorTab({ tab }: { tab: CodingTab }) {
  ...
  useEffect(() => {
    ...
    editor.updateOptions({
      readOnly: fileState.binary || fileState.tooLarge || !!fileState.error,
    })
  }, [
    fileState?.content,
    fileState?.loading,
  ...
  if (fileState?.conflict) {
    return (
      <div className="flex h-full w-full flex-col items-center justify-center gap-2 px-6 text-center text-[12px] text-koma-dim">
  ...
  return (
    <div className="relative h-full w-full">
      ...
      {fileState?.saving && (
        <div className="pointer-events-none absolute right-2 top-8 text-koma-dim">
          <BrailleSpinner size={14} className="opacity-70" />
        </div>
      )}
    </div>
