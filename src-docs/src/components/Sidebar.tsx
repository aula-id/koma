...
type Product = 'welcome' | 'tui' | 'gui'

// ── Navigation data ─────────────────────────────────────────────────────────

const welcomeSections: SidebarSection[] = [
  {
    title: 'Welcome',
    items: [
...
const tuiSections: SidebarSection[] = [
  {
    title: 'TUI',
    items: [
      { label: 'Overview', to: '/tui' },
    ],
  },
  {
    title: 'Tutorials',
...
const guiSections: SidebarSection[] = [
  {
    title: 'GUI',
    items: [
      { label: 'Overview', to: '/gui' },
    ],
  },
  {
    title: 'Tutorials',
...
function detectProduct(pathname: string): Product {
  ...
  if (pathname.startsWith('/gui')) return 'gui'
  return 'welcome'
}
...
export function Sidebar() {
  ...
  const sections = product === 'welcome' ? welcomeSections : product === 'tui' ? tuiSections : guiSections
  ...
  const productLabel = (p: Product) => p === 'welcome' ? 'Welcome' : p === 'tui' ? 'TUI' : 'GUI'
  const productTo = (p: Product) => p === 'welcome' ? '/welcome' : `/${p}`
    ...
    <aside className="flex h-full w-60 flex-none flex-col overflow-hidden border-r border-koma-border">
      ...
      <div className="flex-none px-3 pt-4 pb-1">
        <div className="flex rounded-md border border-koma-border bg-koma-panel p-0.5">
          {(['welcome', 'tui', 'gui'] as Product[]).map((p) => (
            <Link
              key={p}
              to={productTo(p)}
