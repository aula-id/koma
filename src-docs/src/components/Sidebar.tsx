  ...
  return (
    <div className="flex-none px-3 pt-3 pb-2">
      <div className="flex items-center rounded-lg border border-koma-border bg-koma-panel px-2.5 py-1.5 text-[0.8125rem]">
        <svg
          className="mr-2 h-3.5 w-3.5 flex-none text-koma-dim"
          viewBox="0 0 24 24"
          fill="none"
          stroke="currentColor"
          strokeWidth="2"
        ...
        {value && (
          <button
            type="button"
            onClick={() => onChange('')}
            className="ml-1 text-koma-dim hover:text-koma-fg"
          >
            &times;
          </button>
        )}
  ...
  return (
    <nav className="flex-1 overflow-y-auto px-2 pb-4 pt-1">
      {sections.map((section) => {
        const isOpen =
          (openMap[section.title] ?? (section.defaultOpen ?? true)) ||
              ...
              <div className="flex flex-col gap-0.5">
                {section.items?.map((item) => (
                  <NavLink
                    ...
                    filtered={query ? filterItem(item, query) : true}
                    onNavigate={onNavigate}
                  />
                ))}
                {section.groups?.map((group) => {
                  const visibleItems = query
                    ? group.items.filter((i) => filterItem(i, query))
                    : group.items
                  if (visibleItems.length === 0) return null
                  return (
                    <div key={group.label} className="mt-2 first:mt-0">
                      <div className="px-2 py-0.5 text-[10px] font-medium text-koma-dim/60">
                        {group.label}
                      </div>
...
function SidebarPanel({
  ...
  return (
    <aside id={id} className={className}>
      <ProductSwitcher active={product.id} onNavigate={onNavigate} />
...
/** Persistent left column — docs layout only (md+). */
export function DesktopSidebar() {
  return (
    <SidebarPanel className="hidden h-full w-56 flex-none flex-col overflow-hidden border-r border-koma-border bg-koma-bg md:flex" />
  )
}

/**
 * Off-canvas drawer + backdrop for &lt;md.
 * Mounted at root so the TopBar menu works on every route (including home).
 */
export function MobileNavDrawer() {
  const location = useLocation()
  const { open, closeNav } = useDocsNav()
  ...
  return (
    <>
      ...
      <SidebarPanel
        id="docs-mobile-nav"
        onNavigate={closeNav}
        className={`fixed top-14 bottom-0 left-0 z-40 flex w-[min(20rem,88vw)] flex-col overflow-hidden border-r border-koma-border bg-koma-bg shadow-xl transition-transform duration-200 ease-out md:hidden ${
...
/** @deprecated use DesktopSidebar + MobileNavDrawer */
