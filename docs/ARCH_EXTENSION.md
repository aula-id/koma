...
| **`src-agent` extension host/broker** | Per-extension process, Unix socket, handshake, framing, pending calls, generation, grant checks, contribution invocation | The iframe UI; the web store UI |
...
The host binds `~/.koma/run/ext-<id>.sock` before spawning the child. It sets:
