...
9. **Recall intent** = last user message + recent body trajectory (not last user line alone). Blob search ranks by term score then **newer** `msg_id`. Message FTS (`search_messages_before`) restores folded non-blob dialogue. A full blob recall replaces any excerpt for the same message; a preview never suppresses full retrieval.
