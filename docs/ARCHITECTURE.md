...
7. **Hot window:** prefer `short_send_tail_n` body messages, grow under `HOT_TAIL_PCT` (25%) of usable up to `HOT_TAIL_MAX_MSGS` (120), **shrink below `tail_n` when over the token budget** (`tail_n` is a preference). Snap trim to a tool-round start (user or assistant — never a dangling tool). Fat tool/assistant bodies on the tail are replaced with a blob snippet + pointer (stored rail keeps the full text).
