...
**Settings (session-level):** `short_send_enabled` (master), `short_send_engage_n` (sticky hold body message count, default 80), `short_send_tail_n` (hot-window floor, default 40), `max_output_tokens` (interactive chat `max_tokens`; `0` = auto endpoint default — 32k general / 256k direct xAI — always clamped so `prompt_est + max_tokens + margin ≤` model context window; prevents mid-turn HTTP 400 on strict OpenAI-compat hosts), `session_goal` / `session_goal_msg_id` / `session_goal_source` (user-owned doctrine only), `session_charter` (immutable kickoff), `session_objective_fp` (last applied effective fingerprint), `sliding_cache` (cold window 300s vs 120s).
